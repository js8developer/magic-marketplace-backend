...
error AfterHoursExperience__DoesNotOwnNFTFromCollection();
error AfterHoursExperience__AllTicketsHaveBeenClaimed();

contract AfterHoursExperience is ERC721 {
    ...
    constructor(address nftCollectionAddress) ERC721("After Hours Ticket", "AHT"){
    ...
    function claimTicketToExperience() public {
        if (!walletHoldsNFT()){ revert AfterHoursExperience__DoesNotOwnNFTFromCollection(); }
        if (ticketsLeftToClaim() < 1){ revert AfterHoursExperience__AllTicketsHaveBeenClaimed(); }
